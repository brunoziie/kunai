...
Kunai.callback = function (lines, args) {
    ...
    const _value = _lines.map(line => {
        return line.trim()
            ...
            .replace(/@keepCookies\(\)/g, 'kunai.write(\'@cookies\', response)')
...
const parseCookiesFromResponse = (response) => {
    if (response && response.headers && response.headers['set-cookie']) {
        return setCookieParse(response).map(c => `${c.name}=${c.value}`).join('; ');
    }
...
Kunai.prototype.write = function (type, key, value) {
    if (type !== null) {
        ...
        if (type === '@cookies') {
            const cookies = parseCookiesFromResponse(key);

            if (cookies) {
                this._env.headers = this._env.headers || {};
                this._env.headers.Cookie = cookies;
            }
